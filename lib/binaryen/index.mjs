...
export const expandModule = (wmod) => {
    ...
    for (let len = wmod.getNumExports(), i = 0; i < len; ++i) {
        const exportInfo = binaryen.getExportInfo(wmod.getExportByIndex(i));
        if (exportInfo.kind === binaryen.ExternalKinds.Function) {
            exportedFunctions.push({export: exportInfo.name, func: convertIdentifier(translateFuncName(exportInfo.value))})
        }
    }
    ...
    for (const f of exportedFunctions) {
        moduleObject.functions[f.func.index].exportKey = f.export;
    }
    session.clear();
